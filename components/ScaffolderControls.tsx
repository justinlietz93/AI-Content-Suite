        ...
        <div className="animate-fade-in-scale space-y-6">
            ...
            <fieldset className="grid grid-cols-2 sm:grid-cols-2 gap-4">
                 <div>
                    ...
                    <select id="script-language" value={settings.language} onChange={e => handleSettingChange('language', e.target.value as ScriptLanguage)} className="w-full px-3 py-2 bg-slate-700 border-border-color rounded-md text-sm">
                ...
                <div>
                    ...
                    <select id="template" value={settings.template} onChange={e => handleSettingChange('template', e.target.value as ProjectTemplate)} className="w-full px-3 py-2 bg-slate-700 border-border-color rounded-md text-sm">
                ...
                <div>
                    ...
                    <select id="package-manager" value={settings.packageManager} onChange={e => handleSettingChange('packageManager', e.target.value as PackageManager)} className="w-full px-3 py-2 bg-slate-700 border-border-color rounded-md text-sm">
                ...
                <div>
                    ...
                    <select id="license" value={settings.license} onChange={e => handleSettingChange('license', e.target.value as License)} className="w-full px-3 py-2 bg-slate-700 border-border-color rounded-md text-sm">
            ...
            <fieldset className="grid grid-cols-2 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="depth" value={settings.depth} onChange={e => handleSettingChange('depth', parseInt(e.target.value, 10))} min="1" max="3" className="w-full px-3 py-2 bg-slate-700 border-border-color rounded-md text-sm" />
                </div>
                <div>
                    <Label htmlFor="criticRounds">Critic Rounds</Label>
                    <input type="number" id="criticRounds" value={settings.criticRounds} onChange={e => handleSettingChange('criticRounds', parseInt(e.target.value, 10))} min="0" max="2" className="w-full px-3 py-2 bg-slate-700 border-border-color rounded-md text-sm" />
                </div>
            </fieldset>

            <fieldset>
                <legend className="block text-xs font-medium text-text-secondary mb-2">Enforced Constraints</legend>
                <div className="bg-slate-700/50 p-3 rounded-lg text-xs text-slate-400 space-y-1">
